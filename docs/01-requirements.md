...
| FR-045 | `Employee` trong `assigneeIds` phản hồi một đề tài vừa được giao (`ASSIGNED`) bằng **`ACKNOWLEDGED`** / **`DECLINED`** / **`CHANGE_REQUESTED`**; mỗi phản hồi là **một bản ghi `project_events` append-only** (`actor`, `time`, `source`, `assigneeId`; `reasonCode` **bắt buộc** với `DECLINED`/`CHANGE_REQUESTED`, `comment` tự do optional — schema lý do là **PROPOSED**, chờ phỏng vấn/GVHD và **không** có nguồn bắt buộc) — `projects.status` **không đổi** và enum vẫn **5 giá trị** (BR-21, `04 §4.6`); người xử lý phản hồi là `Admin` **trong cùng `departmentId`** (BR-22) | đề tài `ASSIGNED`; **người gọi phải thuộc `assigneeIds`, bất kể `role`** — Admin **không** phản hồi thay Employee | `[UI]` mở đề tài → chọn một trong ba (`response ∈ {ACKNOWLEDGE, DECLINE, REQUEST_CHANGE}`, mapping 1–1 sang event ở `06 §2.4`) → **confirm** (S8) → `insertOne` `project_events` → `notification:new` cho `Admin` cùng `departmentId` → `[UI]` Admin **xác nhận lại để đóng phản hồi** (xem cột ngoại lệ) | **`DECLINED` không gỡ `assigneeIds`**, không đổi `status`; người được giao **vẫn chịu trách nhiệm** tới khi Admin xử lý (BR-21, `19-…` §2.1); **đường reassign/resolution chưa tồn tại**: `POST /projects/:id/assign` chỉ hợp lệ ở `status = DRAFT` nên **không** gán lại được đề tài đang `ASSIGNED` → **Q-10, chặn thiết kế Ticket/Project integration** (`04 §9`), và phản hồi đến trước/sau mốc gán lại là **Q-11** (stale-response race); **không phản hồi** → nhắc 1 lần/ngày có dedupe (BR-12), quá ngưỡng **`TBD`** ngày `[CẦN NGUỒN]` thì **báo Admin** — im lặng **không** là đồng ý; chatbot **chưa có tool** cho ba ý định này (`18` intent `#12a..#12c` ghi `— chưa có tool`) → baseline thao tác trên dashboard, **không tự đặt tên tool mới** (FR-022) | Given đề tài `ASSIGNED` và một phản hồi `DECLINED` đã confirm, When kiểm tra, Then `projects.status` vẫn `ASSIGNED`, có đúng **một** `project_events` loại `DECLINED` kèm `actor`/`time`/`source`, `assigneeIds` **không đổi**, và **không** có giá trị enum thứ 6 nào được ghi; Given người gọi **không** nằm trong `assigneeIds` (kể cả Admin), Then `NOT_ASSIGNEE` (403) và **không** ghi event; Given Admin của phòng ban khác bấm xử lý phản hồi, Then `RBAC_DENIED` (BR-22, FR-057) | `11 §5` dòng F2 (ba nhánh phản hồi + nhánh không phản hồi + `NOT_ASSIGNEE`); `11 §3` (enum 5 giá trị — test phủ định); `11 §4.2` "tool ghi có bước xác nhận" + "RBAC check mỗi tool" |
...
này dành riêng cho mục còn chờ `RP §7.8`. Phần *chưa chốt* của nhánh này gồm: ngưỡng ngày nhắc (`TBD`), tên
tool chatbot mới (§8 mục 12), **schema lý do `reasonCode`/`comment` (PROPOSED)** và **đường reassign/resolution
(Q-10) + stale-response race (Q-11)** — ba mục sau ghi ở `04 §9` và `06 §8` (D-15/D-16). Vì vậy Q-02 **chỉ**
được ghi là **PARTIALLY RESOLVED**, **không** phải đã đóng.
