...
**Python/FastAPI cho lõi AI** (`RESEARCH-PLAN.md` §1; `README.md`). Phần "khoa học" của
...
2. `RESEARCH-PLAN.md` §3 B9 nêu rõ mục tiêu "chạy suy luận ở process riêng để không block event
   loop" — suy luận synchronous trong Node sẽ giữ event loop và làm hỏng `CRUD read p95 < 300 ms` (§B9).
3. Hàng rào eval bắt buộc chạy `pytest` (`RESEARCH-PLAN.md` §11; `NOTES-01.md` §B10 dòng 397).
...
tận "sync hay async job" cho từng endpoint — câu hỏi này được `RESEARCH-PLAN.md` §3 B2 nêu ra và
...
| Gộp `ai-service` vào cùng process với Express bằng child process | Không có bằng chứng trong nguồn; mất lợi ích restart/tài nguyên tách bạch và làm mất ý nghĩa "process riêng để không block event loop" (`RESEARCH-PLAN.md` §3 B9) |
| Ollama / model local thay provider ngoài | Được `RESEARCH-PLAN.md` §3 B6 nêu như một lựa chọn "chọn model + nơi chạy" nhưng NOTES-01 chốt baseline là **Gemini 3.1 Flash-Lite hoặc Groq** (§B6 dòng 305) — model nền tảng cần `transformers`/Python, không phải LLM chat |
...
- Cache embedding "theo skill-string" và warm-up model (`RESEARCH-PLAN.md` §3 B9) là việc
  nội bộ của Python; Node không cần biết.
- Là chỗ duy nhất được phép trả vector ⇒ gate "ai-service không import model của API"
 (`RESEARCH-PLAN.md` §11) kiểm chứng được bằng lệnh.
...
- **Trần RAM chưa biết.** Câu hỏi "VPS Ubuntu 2GB RAM có đủ chạy Node API + Python FastAPI + PhoBERT
 inference trên CPU không? Cần bao nhiêu RAM?" (`RESEARCH-PLAN.md` §3 B1) **chưa có đáp án
...
- **Auth nội bộ giữa hai service chưa chốt:** `RESEARCH-PLAN.md` §3 B2 gợi ý "shared
...
- `docs/research/RESEARCH-PLAN.md` §1; §3 B1; §3 B2; §3 B9; §11
